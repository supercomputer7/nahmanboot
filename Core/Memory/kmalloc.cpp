...
static Core::PhysicalRange s_biggest_available_memory_range;
static PhysicalAddress s_main_bump_allocation_ptr;
bool s_emergency_kmalloc = false;

static u64 s_kmalloc_call_count = 0;
...
void kmalloc_init()
{
    VERIFY(Core::MemoryManagement::the().was_initialized());
    auto biggest_available_memory_range_candidate = Core::MemoryManagement::the().try_to_find_the_biggest_available_range();
    ...
    dbgln("heap main bump allocation start at {}", s_main_bump_allocation_ptr);
}
...
void* kmalloc(size_t size)
{
    if (s_main_bump_allocation_ptr.is_null() || s_emergency_kmalloc) {
        ...
        s_main_bump_allocation_ptr = s_main_bump_allocation_ptr.deoffset(size);
